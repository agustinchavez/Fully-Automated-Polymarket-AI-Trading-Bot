...
def _make_ensemble_config(**kwargs) -> EnsembleConfig:
    defaults = dict(
        ...
        weights={"gpt-4o": 0.40, "claude-sonnet-4-6": 0.35, "gemini-2.5-flash": 0.25},
...
class TestAdaptiveWeightsAutoSwitch:
    ...
    def test_trimmed_mean_with_adaptive_weights_switches_to_weighted(self) -> None:
        ...
        weights = {"gpt-4o": 0.5, "claude-sonnet-4-6": 0.3, "gemini-2.5-flash": 0.2}
        forecaster.set_adaptive_weights(weights)

        model_probs = [("gpt-4o", 0.8), ("claude-sonnet-4-6", 0.6), ("gemini-2.5-flash", 0.4)]
